...
# Set the page configuration and Seaborn style
st.set_page_config(
    page_title="Diabetes Health & Prediction App",
    layout="wide",
    initial_sidebar_state="expanded"
...
@st.cache_data
def load_data():
    """Load the dataset and preprocess for visualization labels."""
    try:
        data = pd.read_csv(DATA_FILE)
        
        # --- Preprocessing for Visualization ---
        data['Diabetes_Status'] = data['Diabetes_012'].map({0.0: 'No Diabetes', 1.0: 'Pre-diabetes', 2.0: 'Diabetes'})
        
        # Age mapping
        age_labels = {1.0: '18-24', 2.0: '25-29', 3.0: '30-34', 4.0: '35-39', 5.0: '40-44', 
        ...
        data['GenHlth_Label'] = data['GenHlth'].map({1.0: 'Excellent', 2.0: 'Very Good', 3.0: 'Good', 4.0: 'Fair', 5.0: 'Poor'})
        ...
        st.error(f"Error: The file '{DATA_FILE}' was not found. Please ensure it is in the same directory as app.py.")
        return pd.DataFrame()
...
if not df.empty and model:
    ...
    with tab2:
        ...
        if submitted:
            ...
            try:
                ...
                st.write("Prediction Probabilities (0: No, 1: Pre, 2: Yes):", prediction_probs)
    ...
    with tab3:
        ...
        st.markdown("These 10 charts highlight key distributions and relationships with diabetes status.")
        
        # Row 1: Distributions & Correlation
        col1, col2 = st.columns(2)
        with col1:
            st.pyplot(plot_1_target_distribution(df))
        with col2:
            st.pyplot(plot_2_top_10_correlation(df))
        ...
        # Row 3: Key Ordinal Factors (Age & General Health)
        ...
        # Row 4: Binary & Behavioral Factors
    ...
    st.warning("Please ensure both `health_diabetes.csv` and `trail_model_diabetes.keras` are correctly placed in the app's directory.")
